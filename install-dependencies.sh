#!/bin/bash
sudo apt-get update
sudo apt-get install -y --no-install-recommends locales ca-certificates nano wget git mercurial screen htop curl openssh-client node-less node-clean-css tig binutils build-essential bzr ca-certificates cpp cups gcc dpkg-dev fontconfig fontconfig-config gir1.2-glib-2.0 git git-core git-man tar zip gzip zlib1g sudo libffi-dev expect-dev libxml2-dev libxslt-dev gcc python2.7-dev libevent-dev libsasl2-dev libldap2-dev libpq-dev libpng12-dev libjpeg-dev xfonts-75dpi xfonts-base postgresql-client* pip virtualenv
